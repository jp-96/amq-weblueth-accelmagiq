...
const requestHeartRateSensor = async (bluetooth: Bluetooth): Promise<BluetoothDevice | undefined> => {
    return await bluetooth.requestDevice({
        ...
        //filters: [{ namePrefix: 'BBC micro:bit', services: [HeartRateService.uuid] }],
        //filters: [{ services: [HeartRateService.uuid] }],
        filters: [{ namePrefix: 'BBC micro:bit'}],
        optionalServices: ['device_information', HeartRateService.uuid]
    });
};
...
const retrieveServices = async (device: BluetoothDevice): Promise<Services> => {
    ...
    return { heartRateService, deviceInformationService };
};
